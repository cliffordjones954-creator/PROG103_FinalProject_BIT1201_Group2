...
def login_screen(root):
    ...
    tk.Label(win, text="Username", bg="#BBDEFB").pack()
    user_entry = tk.Entry(win)
    user_entry.pack()

    tk.Label(win, text="Password", bg="#BBDEFB").pack()
...
def registration_tab(f):
    f.configure(bg="#E8F5E9")
    n, c, a = tk.Entry(f), tk.Entry(f), tk.Entry(f)

    for i, (lbl, e) in enumerate([("Name", n), ("Contact", c), ("Age", a)]):
        tk.Label(
            f,
            text=lbl,
            bg="#E8F5E9",
    ...
    tk.Button(
        ...
        command=lambda: [
            run_query(
                "INSERT INTO Patients(name,contact,age) VALUES(?,?,?)",
                (n.get(), c.get(), a.get())
            ),
    ...
    s = tk.Entry(f)
    tk.Label(f, text="Patient ID").grid(row=4, column=0, padx=5, pady=5)
    s.grid(row=4, column=1, padx=5, pady=5)

    tk.Button(
        f,
        text="Search",
        command=lambda: messagebox.showinfo("Result", search_patient(s.get()) or "Not found")
    ).grid(row=5, column=0, columnspan=2, pady=10)


def booking_tab(frame_booking):
    frame_booking.configure(bg="#FFF3E0")
    # Patient ID
    tk.Label(
    frame_booking,
    text="Patient ID",
    ...
    tk.Button(frame_booking, text="Book Appointment", command=handle_booking,bg="#4CAF50",fg="white",font=("Arial",10,"bold")).grid(row=5, column=0, columnspan=2, pady=10)
    ...
    tk.Button(frame_booking, text="Reschedule Appointment", command=handle_reschedule,bg="#FF9800",fg="white",font=("Arial",10,"bold")).grid(row=9, column=0, columnspan=2, pady=10)
    ...
    tk.Button(frame_booking, text="Cancel Appointment", command=handle_cancel, bg="#F44336",fg="white",font=("Arial",10,"bold")).grid(row=10, column=0, columnspan=2, pady=10)

def confirmation_tab(f):
    f.configure(bg="#F3E5F5")
    ...
    tk.Button(f,text="Refresh",command=refresh,bg="#9C27B0",fg="white",font=("Arial",10,"bold")).pack(pady=10)

#  Main GUI 
def main_gui(root):
    nb = ttk.Notebook(root)
    for tab,name,func in [
        (tk.Frame(nb),"Register",registration_tab),
        (tk.Frame(nb),"Appointments",booking_tab),
        (tk.Frame(nb),"Confirmation",confirmation_tab)
    ]:
    ...
    style.configure("TNotebook.Tab", background="#90CAF9", foreground="black", padding=[10,5])
